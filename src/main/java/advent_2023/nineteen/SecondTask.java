...
public class SecondTask {
    ...
    record Range(int min, int max) {
        public Pair splitBySmaller(int value) {
            if(min >= value) {
                return new Pair(null, new Range(min, max));
            } else if(max < value) {
                return new Pair(new Range(min, max), null);
            } else {
                return new Pair(new Range(min, value - 1), new Range(value, max));
            }
        }

        public Pair splitByLarger(int value) {
            if(min > value) {
                return new Pair(new Range(min, max), null);
            } else if(max <= value) {
                return new Pair(null, new Range(min, max));
            } else {
                return new Pair(new Range(value + 1, max), new Range(min, value));
            }
        }
    }

    record Ranges(Range x, Range m, Range a, Range s) {}
    ...
    public static void main(String[] args) {
        ...
        var ranges = new Ranges(new Range(1, 4000), new Range(1, 4000), new Range(1, 4000), new Range(1, 4000));

        var resultRanges = new ArrayList<Ranges>();

        try {
            checkWorkflows("in", ranges, workflowMap, resultRanges);
        } catch (Exception e) {
        ...
        for(var resultRange : resultRanges) {
            long v = 1;

            v *= resultRange.x.max - resultRange.x.min + 1;
            v *= resultRange.m.max - resultRange.m.min + 1;
            v *= resultRange.a.max - resultRange.a.min + 1;
            v *= resultRange.s.max - resultRange.s.min + 1;
    ...
    private static void checkWorkflows(final String currentWorkflow,
                                       Ranges ranges,
                                       final Map<String, String> workflowMap,
                                       final ArrayList<Ranges> resultRanges) {
        var workflow = workflowMap.get(currentWorkflow);
        var steps = workflow.split(",");

        STEP_LOOP: for(var step : steps) {
            if(step.contains(":")) {
                ...
                if(condition.contains("<")) {
                    ...
                    switch (field) {
                        case "x" -> {
                            var xRanges = ranges.x.splitBySmaller(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(left, ranges.m, ranges.a, ranges.s);
                                if(nextWorkflow.equals("A")) {
                                    resultRanges.add(newRanges);
                                } else if(!nextWorkflow.equals("R")) {
                                    checkWorkflows(nextWorkflow, newRanges, workflowMap, resultRanges);
                                }
                                if(right == null) {
                                    break STEP_LOOP;
                                }
                            ...
                            if (right != null) {
                                ranges = new Ranges(right, ranges.m, ranges.a, ranges.s);
                            }
                        }
                        case "m" -> {
                            var xRanges = ranges.m.splitBySmaller(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, left, ranges.a, ranges.s);
                                if(nextWorkflow.equals("A")) {
                                    resultRanges.add(newRanges);
                                } else if(!nextWorkflow.equals("R")) {
                                    checkWorkflows(nextWorkflow, newRanges, workflowMap, resultRanges);
                            ...
                            if (right != null) {
                                ranges = new Ranges(ranges.x, right, ranges.a, ranges.s);
                            }
                        }
                        case "a" -> {
                            var xRanges = ranges.a.splitBySmaller(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, ranges.m, left, ranges.s);
                                if(nextWorkflow.equals("A")) {
                                    resultRanges.add(newRanges);
                                } else if(!nextWorkflow.equals("R")) {
                                    checkWorkflows(nextWorkflow, newRanges, workflowMap, resultRanges);
                            ...
                            if (right != null) {
                                ranges = new Ranges(ranges.x, ranges.m, right, ranges.s);
                            }
                        }
                        case "s" -> {
                            var xRanges = ranges.s.splitBySmaller(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, ranges.m, ranges.a, left);
                                if(nextWorkflow.equals("A")) {
                                    resultRanges.add(newRanges);
                                } else if(!nextWorkflow.equals("R")) {
                                    checkWorkflows(nextWorkflow, newRanges, workflowMap, resultRanges);
                            ...
                            if (right != null) {
                                ranges = new Ranges(ranges.x, ranges.m, ranges.a, right);
                            }
                        }
                    }
                }
                    ...
                    switch (field) {
                        case "x" -> {
                            var xRanges = ranges.x.splitByLarger(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(left, ranges.m, ranges.a, ranges.s);
                        ...
                        case "m" -> {
                            var xRanges = ranges.m.splitByLarger(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, left, ranges.a, ranges.s);
                        ...
                        case "a" -> {
                            var xRanges = ranges.a.splitByLarger(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, ranges.m, left, ranges.s);
                        ...
                        case "s" -> {
                            var xRanges = ranges.s.splitByLarger(value);
                            var left = xRanges.left;
                            var right = xRanges.right;
                            if(left != null) {
                                var newRanges = new Ranges(ranges.x, ranges.m, ranges.a, left);
                ...
                if(!step.equals("R") && !step.equals("A")) {
                    checkWorkflows(step, ranges, workflowMap, resultRanges);
                }
                if(step.equals("A")) {
                    resultRanges.add(ranges);
                }
            }
        }
    }

...
...........579....595...........8................................781...............................547......................................
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                ...
                var adjacentPartNumbers = new ArrayList<String>();

                boolean[] used = new boolean[8];

                var first = checkIfIsPartNumber(lines, i - 1, j - 1, rows, columns);
                if (first != null) {
                    adjacentPartNumbers.add(first);
                    System.out.println("Found 1 " + first);

                    used[0] = true;
                    used[1] = true;
                    if(Character.isDigit(lines[i-1].charAt(j))) {
                        used[2] = true;
                    }
                }
                var second = checkIfIsPartNumber(lines, i - 1, j, rows, columns);
                if (second != null && !used[1]) {
                   adjacentPartNumbers.add(second);
                    System.out.println("Found 2 " + second);
                    used[2] = true;
                }
                var third = checkIfIsPartNumber(lines, i - 1, j + 1, rows, columns);
                if (third != null&& !used[2]) {
                    adjacentPartNumbers.add(third);
                    System.out.println("Found 3 " + third);
                }
                var fourth = checkIfIsPartNumber(lines, i, j - 1, rows, columns);
                if (fourth != null&& !used[3]) {
                    adjacentPartNumbers.add(fourth);
                    System.out.println("Found 4 " + fourth);
                }
                var fifth = checkIfIsPartNumber(lines, i, j + 1, rows, columns);
                if (fifth != null && !used[4]) {
                    adjacentPartNumbers.add(fifth);
                    System.out.println("Found 5 " + fifth);
                }
                var sixth = checkIfIsPartNumber(lines, i + 1, j - 1, rows, columns);
                if (sixth != null && !used[5]) {
                    adjacentPartNumbers.add(sixth);
                    System.out.println("Found 6 " + sixth);
                    used[5] = true;
                    used[6] = true;
                    if(Character.isDigit(lines[i+1].charAt(j))) {
                        used[7] = true;
                ...
                var seventh = checkIfIsPartNumber(lines, i + 1, j, rows, columns);
                if (seventh != null && !used[6]) {
                    adjacentPartNumbers.add(seventh);
                    System.out.println("Found 7 " + seventh);
                    used[7] = true;
                }
                var eighth = checkIfIsPartNumber(lines, i + 1, j + 1, rows, columns);
                if (eighth != null && !used[7]) {
                    adjacentPartNumbers.add(eighth);
                    System.out.println("Found 8 " + eighth);
                }

                System.out.println("Adjacent part numbers: " + adjacentPartNumbers);

                if(adjacentPartNumbers.size() == 2) {
                    System.out.println("Found " + adjacentPartNumbers.get(0) + " and " + adjacentPartNumbers.get(1));

                    sum += Integer.parseInt(adjacentPartNumbers.get(0)) * Integer.parseInt(adjacentPartNumbers.get(1));
                }
            }
        }
    ...
    private static String checkIfIsPartNumber(String[] lines, int i, int j, int rows, int columns) {
        ...
        if (Character.isDigit(lines[i].charAt(j))) {
            var hasAdjacent = i + 1 < rows && lines[i + 1].charAt(j) != '.';

            if (j + 1 < columns && lines[i].charAt(j + 1) != '.' && !Character.isDigit(lines[i].charAt(j + 1))) {
                hasAdjacent = true;
            }

            if (i - 1 >= 0 && lines[i - 1].charAt(j) != '.') {
                hasAdjacent = true;
            }

            if (j - 1 >= 0 && lines[i].charAt(j - 1) != '.' && !Character.isDigit(lines[i].charAt(j - 1))) {
                hasAdjacent = true;
            }

            if (i + 1 < rows && j + 1 < columns && lines[i + 1].charAt(j + 1) != '.') {
                hasAdjacent = true;
            }

            if (i + 1 < rows && j - 1 >= 0 && lines[i + 1].charAt(j - 1) != '.') {
                hasAdjacent = true;
            }

            if (i - 1 >= 0 && j + 1 < columns && lines[i - 1].charAt(j + 1) != '.') {
                hasAdjacent = true;
            }

            if (i - 1 >= 0 && j - 1 >= 0 && lines[i - 1].charAt(j - 1) != '.') {
                hasAdjacent = true;
            }

            if (hasAdjacent) {
                while (j >= 0 && Character.isDigit(lines[i].charAt(j)) ) {
                    j--;
                }
                ...
                while (j < columns && Character.isDigit(lines[i].charAt(j))) {
                    digit += lines[i].charAt(j);
                    j++;
                }

//                System.out.println("Found " + digit + " at " + i + " " + j);

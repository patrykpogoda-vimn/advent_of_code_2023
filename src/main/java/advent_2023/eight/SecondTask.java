...
public class SecondTask {
    public static void main(String[] args) {
        ...
        for (int i = 2; i < lines.length; i++) {
            ...
            var currentItem = parts[0].trim();
            var pair = parts[1].trim().substring(1, parts[1].length() - 1).split(", ");

            map.put(currentItem, new Pair(pair[0].trim(), pair[1].trim()));
        }

        System.out.println(map);

        var currentFields = map.keySet().parallelStream().filter(s -> s.endsWith("A")).toList();
        var solutionOfEachField = currentFields.parallelStream().map(current -> {
            var currentIterator = current;
            var currentSteps = 0L;

            while (!currentIterator.endsWith("Z")) {
                var pair = map.get(currentIterator);
                var left = pair.left();
                var right = pair.right();

                if (directions.charAt((int)currentSteps % directions.length()) == 'L') {
                    currentIterator = left;
                } else {
        ...
        System.out.println(lcm(solutionOfEachField));
    }

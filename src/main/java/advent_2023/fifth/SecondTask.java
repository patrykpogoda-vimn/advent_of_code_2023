...
public class SecondTask {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            var seedsPairs = Arrays.stream(groups[0].substring(7).split(" "))
                    .map(Long::parseLong)
                    .toList();

            var seedsAndRange = new ArrayList<SeedsAndRange>();

            for (int i = 0; i < seedsPairs.size(); i += 2) {
                var seed = seedsPairs.get(i);
                var count = seedsPairs.get(i + 1);
                seedsAndRange.add(new SeedsAndRange(seed, count));
            }
            ...
            var mappedToSoil = map(seedsAndRange, seedToSoilMap);
            ...
            var lowestLocation = mappedToLocation.stream()
                    .map(SeedsAndRange::seed)
    ...
    record SeedsAndRange(long seed, long range) {}

    private static List<SeedsAndRange> map(List<SeedsAndRange> seeds, List<MapToAndRange> seedMap) {
        List<SeedsAndRange> list = new ArrayList<>();
        for (SeedsAndRange seedAndRange : seeds) {
            list.addAll(produceIntersections(seedAndRange, seedMap));
        }
        return list;
    }

    record Intersections(List<SeedsAndRange> left, List<SeedsAndRange> finished) {

    }

    private static List<SeedsAndRange> produceIntersections(SeedsAndRange seedAndRange, List<MapToAndRange> seedMap) {
        ArrayList<SeedsAndRange> finished = new ArrayList<>();
        ArrayList<SeedsAndRange> todo = new ArrayList<>();
        todo.add(seedAndRange);

        for (MapToAndRange mapToAndRange : seedMap) {
            var todoCopy = new ArrayList<>(todo);
            for(var current: todoCopy) {
                var intersections = produceIntersection(current, mapToAndRange);
                finished.addAll(intersections.finished());
                todo = new ArrayList<>(intersections.left());
            }
        }

        System.out.println("Seed: " + seedAndRange.seed() + " Range: " + seedAndRange.range());
        System.out.println("Seed map: " + seedMap);
        ArrayList<SeedsAndRange> intersections = new ArrayList<>();
        intersections.addAll(todo);
        intersections.addAll(finished);
        System.out.println("Produced intersections: " + intersections);

        return intersections;
    }

    private static Intersections produceIntersection(SeedsAndRange seedAndRange, MapToAndRange mapToAndRange) {
        var seedStart = seedAndRange.seed();
        var seedEnd = seedAndRange.seed() + seedAndRange.range();
        var mapStart = mapToAndRange.start();
        var mapEnd = mapToAndRange.start() + mapToAndRange.range();

        if(seedStart >= mapStart) {
            if(seedStart >= mapEnd) {
                return new Intersections(List.of(seedAndRange), List.of());
            } else {
                var diff = seedStart - mapStart;
                if(seedEnd <= mapEnd) {
                    return new Intersections(
                            List.of(),
                            List.of(new SeedsAndRange(mapToAndRange.mapTo() + diff, seedAndRange.range()))
                    );
                } else {
                    var length = mapEnd - seedStart;

                    return new Intersections(
                            List.of(
                                    new SeedsAndRange(mapEnd, seedEnd - mapEnd)
                            ),
                            List.of(
                                    new SeedsAndRange(mapToAndRange.mapTo() + diff, length)
                            )
                    );
                }
            }
            ...
            if(seedEnd <= mapStart) {
                ...
                var diff = mapStart - seedStart;
                if(seedEnd <= mapEnd) {
                    return new Intersections(
                            List.of(
                                    new SeedsAndRange(seedStart, diff)
                            ),
                            List.of(
                                    new SeedsAndRange(mapToAndRange.mapTo(), seedEnd - mapStart)
                            )
                    );
                } else {
                    return new Intersections(
                            ...
                            List.of(
                                    new SeedsAndRange(mapToAndRange.mapTo() + mapToAndRange.range(), mapEnd - mapStart)
                            )
                    );
                }
            }
    ...
    record MapToAndRange(long start, long mapTo, long range) {}

    private static List<MapToAndRange> getList(String[] groups, int x) {
        return Arrays.stream(groups[x].substring(groups[x].indexOf("\n") + 1)
                        .split("\n"))
                .map(s -> Arrays.stream(s.split(" ")).map(Long::parseLong).toList())
                .map(l -> new MapToAndRange(l.get(1), l.get(0), l.get(2)))
                .toList();
    }
}

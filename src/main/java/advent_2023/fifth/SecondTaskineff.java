...
public class SecondTaskineff {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            var lowestLocation = mappedToLocation.stream().map(seedAndRange -> seedAndRange.seed()).min(Long::compareTo).get();
    ...
    private static List<SeedsAndRange> map(List<SeedsAndRange> seedsAndRanges, List<List<Long>> seedMap) {

        return seedsAndRanges.stream().map(
                seedAndRange -> {
                    var seeds = new ArrayList<Long>();

                    for (long i = seedAndRange.seed(); i < seedAndRange.range(); i++) {
                        seeds.add(seedAndRange.seed());
                    }

                    var mappedSeeds = seeds.stream()
                            .map(seed -> {
                                for (var map : seedMap) {
                                    var  mapTo = map.get(0);
                                    var start = map.get(1);
                    ...
                    return new SeedsAndRange(mappedSeeds.get(0), mappedSeeds.size());
                }
        ).toList();
    }
